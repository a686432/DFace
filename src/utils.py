...
def Q2R_batch(Q):
    ...
    R = (
        torch.stack([w1, w2, w3, w4, w5, w6, w7, w8, w9])
        .transpose(1, 0)
...
def matrix2angle(R):
    ...
    b1 = ((R[:, 2, 0] != 1) + (R[:, 2, 0] != -1)).eq(2)
    b2 = ((1 - b1) + (R[:, 2, 0] == -1)).eq(2)
    b3 = ((1 - b1) + (R[:, 2, 0] != -1)).eq(2)
...
def E2Q(fi, xita, psi):
    ...
    return :
            Q: Tensor (4, ) x, y, z, s
    """
    x = torch.sin(fi / 2) * torch.cos(xita / 2) * torch.cos(psi / 2) - torch.cos(
        fi / 2
    ) * torch.sin(xita / 2) * torch.sin(psi / 2)
    y = torch.cos(fi / 2) * torch.sin(xita / 2) * torch.cos(psi / 2) + torch.sin(
        fi / 2
    ) * torch.cos(xita / 2) * torch.sin(psi / 2)
    z = torch.cos(fi / 2) * torch.cos(xita / 2) * torch.sin(psi / 2) - torch.sin(
        fi / 2
    ) * torch.sin(xita / 2) * torch.cos(psi / 2)
    s = torch.cos(fi / 2) * torch.cos(xita / 2) * torch.cos(psi / 2) + torch.sin(
        fi / 2
    ) * torch.sin(xita / 2) * torch.sin(psi / 2)
...
def rodrigues_rotation(points, rod):
    ...
    b = (
        (1 - cos(rod[3]))
        * (torch.mm(points.transpose(0, 1), rod[0:3]).squeeze(1))
        * rod[0:3]
    )
