...
class BFMG_batch(nn.Module):
    def __init__(self):
        ...
        w_expression_tensor = (
            torch.Tensor(bfm["w_expression"]).reshape(-1, 3, 29).reshape(-1, 29).t()
            / 1000
        )
        mu_expression_tensor = (
            torch.Tensor(bfm["mu_expression"]).reshape(-1, 3).reshape(-1) / 1000
        )
        # print(mu_expression)
        # print(w_expression @ w_expression.t())
        #########################################################################
        g_model = np.load(config.g_model_path)
        # print(g_model['landmarks'])
        outer_landmark_tensor = torch.Tensor(g_model["landmarks"].astype(int)).long()

        # x = torch.Tensor(f_model['x'])
        # print(x.norm(dim=1).shape)
        mu_shape_tensor = torch.Tensor(g_model["mu"])
        w_tensor = torch.Tensor(g_model["w_shape"])
        index = torch.Tensor(g_model["faces"])
        if config.re_type_gan:
            shape_ev_tensor = torch.Tensor(bfm["shape_ev"])
        else:
            shape_ev_tensor = torch.Tensor(g_model["shape_ev"])
        print(outer_landmark_tensor)
        # exit()
        # print(mu_expression_tensor, mu_shape_tensor)
        mu_tensor = mu_shape_tensor + mu_expression_tensor
        exp_ev_tensor = torch.Tensor(bfm["exp_ev"])
        # shape_ev_tensor = torch.Tensor(bfm['shape_ev'])
        # landmark = np.load('../propressing/landmark.npy')
        # print(landmark)
        # outer_landmark_tensor = torch.Tensor(landmark[landmark!=-1].astype(np.int32)).long()
        # print(list(bfm.keys()))
        # nn.parameters
        ...
        self.aflw_21_landmarks = np.array(
            [
                37535,
                4645,
                38561,
        ...
        self.register_buffer("tris", index)
    ...
    def forward(self, shape_para, exp_para, camera_para=None):
        ...
        # print(self.mu)
        face_vertex_tensor = self.mu + shape_para @ self.w_shape + exp_para @ self.w_exp
        ...
        rotated = torch.bmm(
            R, face_vertex_tensor.reshape(batch_size, -1, 3).transpose(2, 1)
        ).reshape(batch_size, -1)
        scaled = (-torch.abs(camera_para[:, 4].unsqueeze(1)) * rotated).reshape(
            batch_size, 3, -1
        )  #
    ...
    def get_outer_moved_landmark(self, face_vertex, camera_para):
        outer_landmark_moved_idx = self._move_boundary_landmarks(
            face_vertex, camera_para
        )
    ...
    def __move_boundary_landmarks(self, face_vertex_tensor, Camera_Para):
        ...
        R = Q2R_batch(
            E2Q_batch(phi, theta, psi)
        )  # E -> Q -> R R shape = (batch_size, 3, 3)
        rotated_vertex = torch.bmm(
            R, face_vertex_tensor.reshape(batch_size, -1, 3).transpose(1, 2)
        ).transpose(1, 2)

        lms = np.append(np.array(range(0, 6)), np.array(range(11, 17)))
        mask = np.array([0, 0, 0, 0, 0, 0, -5, -5, -5, -5, -5, -5])

        landmark_vertex_cand = rotated_vertex[
            :, self.outer_landmark_index_cand[lms + mask]
        ]
        ...
        outer_landmark_moved[:, lms] = self.outer_landmark_index_cand[
            lms + mask, selected[:, :, 0]
        ]
    ...
    def get_tikhonov_regularization(self, Shape_Para, Exp_Para):
        ...
        return config.tik_shape_weight * torch.sum(a ** 2, dim=1) + torch.sum(
            b ** 2, dim=1
        )
    ...
    def get_2d_landmark_loss(self, gt_landmark):
        ...
        self.projected = (
            self.project_outer_landmark()
        )  # get the projected 68 landmarks, shape = (batch_size, 2, 68)
        self.projected = self.projected.transpose(1, 2).reshape(
            self.batch_size, -1
        )  # flatten it
    ...
    def project_outer_landmark(self, use_aflw=False, use_not_moved=False):
        ...
        if use_not_moved:
            rotated = torch.bmm(
                R, self.outer_landmark_not_moved_tensor.transpose(2, 1)
            ).reshape(self.batch_size, -1)
        else:
            rotated = torch.bmm(
                R,
                self.outerlandmark_vertex_tensor
                if not use_aflw
                else self.aflw_21_landmark_vertex_tensor,
            ).reshape(self.batch_size, -1)
        # print 'camera scale = ', torch.abs(self.Camera_Para[:, 4]).unsqueeze(1)
        scaled = (-torch.abs(self.Camera_Para[:, 4].unsqueeze(1)) * rotated).reshape(
            self.batch_size, 3, -1
        )
    ...
    def transform_face(self):
        ...
        R = Q2R_batch(self.Camera_Para[:, 0:4])
        rotated = torch.bmm(
            R, self.face_vertex_tensor.reshape(self.batch_size, -1, 3).transpose(2, 1)
        ).reshape(
            self.batch_size, -1
        )  # batchszize * N
        scaled = (-torch.abs(self.Camera_Para[:, 4].unsqueeze(1)) * rotated).reshape(
        ...
        scaled[:, :2, :] = scaled[:, :2, :] + self.Camera_Para[:, 5:7].view(
            self.batch_size, 2, 1
        )
