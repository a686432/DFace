...
def mega_feature(
    model, eval_loader_facescrub, eval_loader_megaface, megaface_out, facescrub_out
):
    model.eval()
    ...
    for batch_idx, (imgnames, data, dataf) in enumerate(eval_loader_facescrub):
        ...
        for idx, imgname in enumerate(imgnames):
            ...
            out_path = os.path.join(
                out_dir, b + "_%s_%dx%d.bin" % ("sphereface", 112, 96)
            )
            feats[idx].tofile(out_path)
            sys.stdout.write(
                str(batch_idx) + "/" + str(len(eval_loader_facescrub)) + "\r"
            )
    ...
    for batch_idx, (imgnames, data, dataf) in enumerate(eval_loader_megaface):
        ...
        for idx, imgname in enumerate(imgnames):
            ...
            feats[idx].tofile(out_path)
            sys.stdout.write(
                str(batch_idx) + "/" + str(len(eval_loader_megaface)) + "\r"
            )


def eval_megaface(
    model, root_facescrub, root_megaface, facescrub_out, megaface_out, fdict
):
    state = torch.load(fdict)
    state_dict = state["dict"]
    model.load_state_dict(state_dict)
    print("Start evaling MegaFace...")
    evalset_facescrub = FeatureDataset(
        root=root_facescrub,
        filelist="files.txt",
        transform=transform_eval,
        target_transform=transform_eval_f,
    )
    eval_loader_facescrub = torch.utils.data.DataLoader(
        evalset_facescrub, batch_size=64, shuffle=False, num_workers=16
    )
    evalset_megaface = FeatureDataset(
        root=root_megaface,
        filelist="files.txt",
        transform=transform_eval,
        target_transform=transform_eval_f,
    )
    eval_loader_megaface = torch.utils.data.DataLoader(
        evalset_megaface, batch_size=64, shuffle=False, num_workers=16
    )
    mega_feature(
        model, eval_loader_facescrub, eval_loader_megaface, megaface_out, facescrub_out
    )
...
def eval_cfp_fp(model, root, fdict=""):
    ...
    evalset = FeatureDataset(
        root,
        filelist="../../file_path_list_cfp_align.txt",
        transform=transform_eval,
        target_transform=transform_eval_f,
    )
    eval_loader = torch.utils.data.DataLoader(
        evalset, batch_size=32, shuffle=False, num_workers=72
    )
    ...
    for i in range(7000):
        ...
        p_feat = np.fromfile(
            os.path.join(root, "%d.bin" % (2 * i + 1)), dtype="float32"
        )
    ...
    print(
        "CFP-FP ACC={:.4f} std={:.4f} thd={:.4f}".format(
...
def eval_cfp(model, root, fdict):
    ...
    evalset = FeatureDataset(
        root="/home/jdq/database/database/cfp_crop",
        filelist="files.txt",
        transform=transform_eval,
        target_transform=transform_eval_f,
    )
    eval_loader = torch.utils.data.DataLoader(
        evalset, batch_size=64, shuffle=False, num_workers=72
    )
    gen_feature(model, eval_loader)
    ...
    def num2path():
        ...
        with open(os.path.join(root, "Protocol/Pair_list_F.txt")) as f:
            ...
            for pair in lines:
                pair = pair[:-1].split()
                pair_list_f[int(pair[0])] = (
                    pair[1].replace(".jpg", "_crop.bin").replace("..", root)
                )
        with open(os.path.join(root, "Protocol/Pair_list_P.txt")) as f:
            lines = f.readlines()
            for pair in lines:
                pair = pair[:-1].split()
                pair_list_p[int(pair[0])] = (
                    pair[1].replace(".jpg", "_crop.bin").replace("..", root)
                )
    ...
    for i in range(1, 11):
        ...
        with open(diff_path) as f:
            ...
            for pair in lines:
                ...
                if os.path.exists(pair_list_f[pair[0]]) and os.path.exists(
                    pair_list_p[pair[1]]
                ):
...
def eval_ytf(model, root, splitfile="../splits_corrected.txt", fdict=None):
    ...
    eval_loader = torch.utils.data.DataLoader(
        evalset, batch_size=64, shuffle=False, num_workers=16
    )
    ...
    for batch_idx, line in enumerate(split_lines):
        ...
        cosdistance = (
            np.dot(mean_feat1, mean_feat2)
            / np.linalg.norm(mean_feat1)
            / np.linalg.norm(mean_feat2)
        )
    ...
    for idx, (train, test) in enumerate(folds):
        ...
        thd.append(best_thresh)
    print(
        "YTFACC={:.4f} std={:.4f} thd={:.4f}".format(
            np.mean(accuracy), np.std(accuracy), np.mean(thd)
        )
    )
...
def eval_lfw(model, root, dicts="", train=False):
    evalset = data_loader.LfwDataSet(
        root=root,
        pairfile="../pair.txt",
        transform=transform_eval,
        target_transform=transform_eval_f,
    ...
    eval_loader = torch.utils.data.DataLoader(
        evalset, batch_size=16, shuffle=False, num_workers=24
    )
    if not train:
        if os.path.isdir(dicts):
            ...
            for dict_file in dict_files:
                if os.path.splitext(dict_file)[-1] == ".cl":
                    ...
                    for batch_idx, (
                        ...
                        data1, data2, data1f, data2f, target = (
                            data1.cuda(),
                            data2.cuda(),
                            data1f.cuda(),
                            data2f.cuda(),
                        ...
                        cosdistance = cosdistance.sum(dim=1) / (
                            ip1.norm(dim=1).reshape(-1) * ip2.norm(dim=1).reshape(-1)
                            + 1e-12
                        )
                        ...
                        labels.append(label)
                        sys.stdout.write(
                            str(batch_idx) + "/" + str(len(eval_loader)) + "\r"
                        )
                        sys.stdout.flush()
                    ...
                    for idx, (train, test) in enumerate(folds):
                        ...
                        accuracy.append(
                            lfw.eval_acc(best_thresh, predicts[test], labels[test])
                        )
                        thd.append(best_thresh)
                    print(
            ...
            predicts = []
            labels = []
            for batch_idx, (_, _2, data1, data2, data1f, data2f, target) in enumerate(
                eval_loader
            ):
                data1, data2, data1f, data2f, target = (
                    data1.cuda(),
...
def compare_two_video(
    model,
    dir1="/home/jdq/database/database/ytf_crop/Uzi_Landau/2",
    dir2="/home/jdq/database/database/ytf_crop/Aaron_Guiel/5",
):
...
class EvalTool(object):
    ...
    def __init__(
        ...
        criterion=None,
        tb_writer=None,
        eval_fr_data_root_path=config.eval_fr_data_root_path,
        transform=None,
        aflw_data_root_path=config.aflw_data_root_path,
    ):
        ...
        (
            ...
            self.cfp_fp_issame,
            self.lfw_issame,
        ) = self._get_fr_val_data(eval_fr_data_root_path)
        ...
        self.eval_loader_aflw = torch.utils.data.DataLoader(
            evalset_aflw, batch_size=batch_size, shuffle=False, num_workers=4
        )
        # self.landmark_map = torch.Tensor(np.load(config.f_model_landmark_map)).long()

        transform_eval_f = transforms.Compose(
            [transforms.RandomHorizontalFlip(p=1), self.tfm]
        )

        print("#####################################################")
        # evalset_ytf = FeatureDataset(root='/data/jdq/ytf_crop/',filelist="files.txt",transform=self.tfm,target_transform=transform_eval_f)
        # # evalset = YTFDataSet(root="/home/jdq/database/database/",pairfile="../splits.txt",transform=transform_eval)
    ...
    def get_micc_rmse(self, model, save_tmp=False):
        ...
        for batch_idx, (data, target, image_name) in enumerate(self.eval_loader_micc):
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=24) as executor:
                futures = [
                    executor.submit(proxy, vert, label)
                    for vert, label in zip(
    ...
    def update_tb(self, model, x_step, eval_ytf=False, emb_idx=1, mask=0):
        if self.writer is not None:
            ...
            if mask & 4:
                aflw_ced = self._evaluate_aflw(model)

                self.writer.add_image("eval_aflw2000_CED", aflw_ced, x_step)
            if mask & 8:
                lfw_acc, lfw_thd, lfw_roc = self._evaluate_fr(
                    model,
                    self.lfw,
                    self.lfw_issame,
                    name="LFW",
                    embedding_size=embedding_size[emb_idx],
                    emb_idx=emb_idx,
                )
                agedb_acc, agedb_thd, agedb_roc = self._evaluate_fr(
                    model,
                    self.agedb_30,
                    self.agedb_30_issame,
                    name="Agedb_30",
                ...
                cfp_acc, cfp_thd, cfp_roc = self._evaluate_fr(
                    model,
                    self.cfp_fp,
                    self.cfp_fp_issame,
                    name="CFP-FP",
    ...
    def _evaluate_fr2(
        ...
        for i in range(len(carray)):
            ...
            embeddings[i] = torch.tensor(
                tmp
            )  # *torch.tensor(BFMA_batch.BFMA_batch.shape_ev[:99]).float()
            # print(embeddings[i])
        ...
        for idx, (train, test) in enumerate(folds):
            ...
            ip1 = torch.tensor(new_em[0::2])
            ip2 = torch.tensor(new_em[1::2])
            ...
            best_thresh = self.find_best_threshold(
                thresholds, predicts[(train / 2)[0::2]], labels[(train / 2)[0::2]]
            )
            tpr, fpr, acc = self._calculate_accuracy(
                best_thresh, predicts[(test / 2)[0::2]], labels[(test / 2)[0::2]]
            )
            print(acc)
            accs.append(acc)
        ...
        for idx, (train, test) in enumerate(folds):
            ...
            tpr, fpr, acc = self._calculate_accuracy(
                best_thresh, predicts[test], labels[test]
            )
            accuracy.append(acc)
            thd.append(best_thresh)
    ...
    def _evaluate_fr(
        ...
        with torch.no_grad():
            while idx + self.batch_size <= len(carray):
                ...
                embeddings[idx : idx + self.batch_size] = pred.cpu()
    ...
    def _evaluate_aflw(self, model, save_path=None, device="cuda"):
        ...
        for batch_idx, (data, target) in enumerate(self.eval_loader_aflw):
            ...
            face_vertex_tensor, rotated, scaled = self.bfmf(
                pred[:, :99], pose_expr[:, 7:36], pose_expr[:, 0:7]
            )
            # print(self.bfmf.module.get_landmark_68(scaled).shape)
            pred_lms = (
                self.bfmf.module.get_landmark_68(scaled)
                .transpose(2, 1)
    ...
    def _eval_ytf(
        self,
        model,
        root="/data/jdq/ytf_crop/",
        splitfile="../splits_corrected.txt",
        fdict=None,
        emb_idx=1,
    ):
        def gen_feature(model, dataloader, to_file=True):
        ...
        predicts = np.array(cosdistance).flatten()
        # print(predicts.shape)
        labels = np.array(labels).flatten()
    ...
    def inference(model, img_path):
        ...
        shape_para = shape[0, 0:99].unsqueeze(1)
...
if __name__ == "__main__":
    ...
    a = EvalTool(transform=tfm, tb_writer=writer)
    import net

    print("Loading model")
    model = net.sphere64a(pretrained=False)

    path = "../model/para_jdq/n1.pkl"
    gpu_ids = range(1)
    ...
    a.get_fr_eval_acc(model, embedding_size=99, emb_idx=1)
