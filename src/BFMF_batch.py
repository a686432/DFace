...
class BFMF_batch(nn.Module):
    def __init__(self):
        ...
        index_map = torch.Tensor(np.load(config.f_model_index_map) - 1).long()
        # w = bfm['w_shape'][:,0:99]
        # #f = open(imagefile,'r')
        # print(w)
        w_expression_tensor = (
            torch.Tensor(bfm["w_expression"])
            .reshape(-1, 3, 29)[index_map, 0:29]
            .reshape(-1, 29)
            .t()
            / 1000
        )
        mu_expression_tensor = (
            torch.Tensor(bfm["mu_expression"]).reshape(-1, 3)[index_map].reshape(-1)
            / 1000
        )
        # print(mu_expression)
        ...
        mu_shape_tensor = torch.Tensor(f_model["mu"])
        w_tensor = torch.Tensor(f_model["w_shape"])
        index = torch.Tensor(f_model["faces"])
        ...
        # print(landmark)
        outer_landmark_tensor = torch.Tensor(
            landmark[landmark != -1].astype(np.int32)
        ).long()
        # print(list(bfm.keys()))
        # nn.parameters
    ...
    def get_batch_distribution_regular_shape(self, shape_para):
        ...
        a = torch.sum((shape_para / 100) ** 2, dim=1).mean()
