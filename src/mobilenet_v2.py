...
class MobileFaceNet(Module):
    ...
    def forward(self, x):
        ...
        out = self.bn(out)
        return l2_norm(out), out[:, :99], out[:, 99:135]
...
def margin_observe(feat, y, num_cls):
    ...
    max_value = torch.min(where(y_onehot, 3.15, feat), dim=1)[0]
    y_value = feat.gather(1, y.data.view(-1, 1))
    margin = (max_value - y_value).mean()
    return margin


class mArcface(Module):
    # implementation of additive margin softmax loss in https://arxiv.org/abs/1801.05599
    def __init__(self, embedding_size=512, classnum=51332, s=32.0, m=0.5):
        super(mArcface, self).__init__()
        self.classnum = classnum
        self.kernel = Parameter(torch.Tensor(embedding_size, classnum))
        # initial kernel
        self.kernel.data.uniform_(-1, 1).renorm_(2, 1, 1e-5).mul_(1e5)
    ...
    def forward(self, embbedings, label):
        ...
        y = label
        feat = torch.acos(cos_theta)
        margin = margin_observe(feat, y, self.classnum)
        ...
        diff = m_now - self.m_h
        dalta = 0.9 * self.dalta + 1 * diff.mean().detach()

        l = 0.99 * self.m_t + 0.01 * m_now.clamp(min=0) + dalta.clamp(min=0)
        l = l.detach()
        ##
        self.m_t = float(l.data.cpu().numpy())
        self.m_h = m_now

        # sys.stdout.write('TMargin={:.4f} | '.format(self.m_t))
        # l=0.99*self.m_t+0.01*m_now.clamp(min=0) #+ 0.1*dalta.clamp(min=0)
        # l = l.detach()
        # ##
        # self.m_t=float(l.data.cpu().numpy())
        # self.m_h = m_now

        self.cos_m = math.cos(self.m_t)
        self.sin_m = math.sin(self.m_t)
        ...
        return output, (self.m, self.m_t)


def mobilenet_2(num_classes=135, input_channel=3):
