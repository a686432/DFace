...
class FLAME(nn.Module):
    ...
    def __init__(self):
        ...
        self.register_buffer('e_shape',torch.ones(config.shape_params,dtype=self.dtype, requires_grad=False))
        self.register_buffer('e_exp',torch.ones(config.expression_params,dtype=self.dtype, requires_grad=False))
        ...
        if self.use_face_contour:
            ...
            self.register_buffer('dynamic_lmk_faces_idx',
                                 dynamic_lmk_faces_idx)
            print(dynamic_lmk_faces_idx.shape)
    ...
    def forward(self, shape_params=None, expression_params=None, pose_params=None, neck_pose=None, eye_pose=None, transl=None):
        ...
        full_pose = torch.cat([pose_params[:,:3], neck_pose, neck_pose, eye_pose], dim=1)
        ...
        scaled = (-torch.abs(pose_params[:, 4].unsqueeze(1)) * vertices).reshape(batch_size, 3, -1)# 
        scaled[:, :2, :] = scaled[:, :2, :] + pose_params[:, 5:7].view(batch_size, 2, 1)
    ...
    def get_landmark_51(self,face_pixels):

        lmk_faces_idx = self.lmk_faces_idx.unsqueeze(dim=0).repeat(self.batch_size, 1)
        lmk_bary_coords = self.lmk_bary_coords.unsqueeze(dim=0).repeat(self.batch_size, 1, 1)
        landmarks = vertices2landmarks(face_pixels, self.faces_tensor,lmk_faces_idx,lmk_bary_coords)
        # print('vertices',face_pixels.shape)
        
        #print('landmarks', landmarks.shape)
        return landmarks[ ..., :2].transpose(1,2)


    def get_edge_landmark_51(self,landmark):
        batch_size = landmark.shape[0]
        landmarks = landmark.transpose(2,1)
        landmarks = landmarks.transpose(0,1).expand(51,51,batch_size,2).transpose(0,2)
