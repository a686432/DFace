...
def eval_acc(threshold, diff, labels):
    y_true = []
    y_predict = []
    for idx,d in enumerate(diff):
        #print(threshold)
        same = 1 if float(d) > threshold else 0
...
def find_best_threshold(thresholds, predicts, labels):
    best_threshold = best_acc = 0
    for threshold in thresholds:
        accuracy = eval_acc(threshold, predicts,labels)
        if accuracy >= best_acc:
            sys.stdout.write('LFWACC={:.4f} std={:.4f} thd={:.4f}\r'.format(np.mean(accuracy), np.std(accuracy), np.mean(threshold)))
            best_acc = accuracy
