...
class BFMN_batch(nn.Module):
    def __init__(self,dim=199):
        super(BFMN_batch, self).__init__()
        """    
        ...
        self.mlc.load_state_dict(torch.load(config.mlc_path))
       ...
       # print(list(bfm.keys()))
        ...
        self.register_buffer('pi',torch.tensor(3.141592653589).float())
    ...
    def forward(self,shape_para, exp_para, camera_para=None,shape_flip=False):
        ...
        if config.use_res_shape:
            ...
            face_shape_res = self.mu +  res_shape
            if shape_flip:
        ...
        face_vertex_tensor = self.mu +  self.mlc(shape_para[:,:self.dim])  + exp_para @ self.w_exp
        #R = Q2R_batch(camera_para[:, 0:4])
    ...
    def get_resloss(self,shape_para):
        ...
        return KLD
    def get_resloss2(self,shape_para):
        res = self.mlc(shape_para[:,self.dim:]) + shape_para[:,:self.dim] @ self.w_shape
        mean_loss = (res.mean(dim=0)**2).mean()
        loss = (((res**2).mean(dim=0)-self.res_var)**2).mean() + mean_loss
        return loss
    ...
    def _shader_tex(self,fragments,meshes,illum=None):
        ...
        #illum = illum.view(-1,9,3).expand(N*H*W*K, 9, 3)
        ...
        # 1
        h1 = la[0] * (1 / torch.sqrt(4 * torch.Tensor([np.pi]))).expand(pn).to(config.device)
        # 2
        h2 = la[1] * np.sqrt(3 / (4 * np.pi)) * pixel_normals[:, 2]
        # 3
        h3 = la[1] * np.sqrt(3 / (4 * np.pi)) * pixel_normals[:, 0]
        # 4
        h4 = la[1] * np.sqrt(3 / (4 * np.pi)) * pixel_normals[:, 1]
        # 5
        h5 = la[2] * 0.5 * np.sqrt(5 / (4 * np.pi)) * (3 * (pixel_normals[:, 2] ** 2) - 1)
        # 6
        h6 = la[2] * 3 * np.sqrt(5 / (12 * np.pi)) * (pixel_normals[:, 0] * pixel_normals[:, 2])
        # 7
        h7 = la[2] * 3 * np.sqrt(5 / (12 * np.pi)) * (pixel_normals[:, 1] * pixel_normals[:, 2])
        # 8
        h8 = la[2] * 1.5 * np.sqrt(5 / (12 * np.pi)) * (pixel_normals[:, 0] **2 -  pixel_normals[:, 1] ** 2)
        # 9
        h9 = la[2] * 3 * np.sqrt(5 / (12 * np.pi)) * (pixel_normals[:, 0] * pixel_normals[:, 1])
        ...
        pix_color =(texels*illum.view(N,H,W,K,C)).clamp(min=0,max=1)
        ...
        return pix_color,illum_colors, texels
    ...
    def get_pho_loss(self, ori_images,proj_face,albedo, illum):
        ...
        images = softmax_rgb_blend(colors, fragments,BlendParams(),zfar=500)
