...
def get_distribution_sampler():
    #print(config.x)
    return lambda b: torch.Tensor(config.gmm_data[np.random.randint(0,6000000,size=b)])
...
def train(epoch):
    ...
    for batch_idx,(data, target, ori_img) in tqdm(enumerate(train_loader), total = len(train_loader)):
        ...
        # if x_step > len(train_loader)*0.5:
        #     config.weight_edge_lm = config.weight_edge_lm_raw

    
        if config.soft_start_pixel_loss:
            if x_step < len(train_loader)*p_th:
                config.weight_pixel_loss = 0
            # elif x_step < len(train_loader)*0.5:
            #     config.weight_pixel_loss = 1.* config.weight_pixel_loss_raw * (x_step-len(train_loader)*0.2) /(len(train_loader)*0.5-len(train_loader)*0.2)
            elif x_step == len(train_loader)*p_th:
                save_model(model, config.result_dir + 'model/train_warm3d_step_%d_' % x_step + config.prefix + '.pkl')
                save_model(criterion, config.result_dir + 'model/train_warm3d_step_criterion_%d_' % x_step + config.prefix + '.pkl')
                save_model_m(optimizer4nn, config.result_dir + 'model/train_warm3d_optimizer4nn_%d_' % x_step + config.prefix + '.pkl')
            else:
                config.weight_pixel_loss = config.weight_pixel_loss_raw

        if config.soft_start_perceptual_loss:
            if x_step < len(train_loader)*pre_th:
                config.weight_loss_perc_im = 0
            elif x_step == len(train_loader)*pre_th:
                save_model(model, config.result_dir + 'model/train_warmpixel_%d_' % x_step + config.prefix + '.pkl')
                save_model(criterion, config.result_dir + 'model/train_train_warmpixel_criterion_%d_' % x_step + config.prefix + '.pkl')
                save_model_m(optimizer4nn, config.result_dir + 'model/train_train_warmpixel_step__optimizer4nn_%d_' % x_step + config.prefix + '.pkl')

            else:
                config.weight_loss_perc_im = config.weight_loss_perc_im_raw

            # save_model(model, config.result_dir + 'model/train_stage3_epoch_%d_' % epoch + config.prefix + '.pkl')
            # save_model(criterion, config.result_dir + 'model/train_stage3_epoch_criterion_%d_' % epoch + config.prefix + '.pkl')
        ...
        # d_index = 0
        # st = time()
        conf , shape, abedlo, camera_exp = model(data)

        # if config.re_type_gan:
        #     regular_loss_o = criterion_B(D(shape), Variable(torch.ones([batch_size,1]).cuda())).mean()
        #     if g_index > config.g_steps and (regular_loss_o<3 or g_index > config.g_steps*5):
        #         d_index = 0 
        ...
        #         while d_index<config.d_steps or (dfe>0.5 and d_index<config.d_steps*2):
        ...
        #             d_real_error = criterion_B(d_real_decision, Variable(torch.ones([batch_size,1]).cuda()))  
        #             d_real_error.mean().backward() # compute/store gradients, but don't change params  
        #             d_optimizer.step()

        #             d_optimizer.zero_grad()
        #             d_fake_decision = D(d_fake_data) 
        #             d_fake_error = criterion_B(d_fake_decision, Variable(torch.zeros([batch_size,1]).cuda()))  # zeros = fake
        #             d_fake_error.mean().backward()
        #             d_optimizer.step()     # Only optimizes D's parameters; changes based on stored gradients from backward()
        #             d_fake_error = criterion_B(d_fake_decision, Variable(torch.zeros([batch_size,1]).cuda()))
        #             dre, dfe = sum(extract(d_real_error))/len(extract(d_real_error)), sum(extract(d_fake_error))/len(extract(d_fake_error))
                #print(dfe)
        ###############################################################################################
        ...
        # st1 = time()-st
        # print('network time:', st1)
        metrics,train_result = criterion(camera_exp, shape, target,abedlo,ori_img,conf)
        ...
        #print(pixel_loss.mean())
        loss = loss_3d.mean() + regular_loss.mean() + config.tik_exp_weight*re_e.mean()+ \
...
if __name__ == "__main__":
    ...
    model = net.sphere64a(pretrained=False,model_root=dict_file, stage = 3)
    if config.start_from_warmpixel:
        load_model(model,config.checkpoint_warm_pixel+'.pkl') 
    ...
    if config.use_face_recognition_constraint:
        ...
        from fr_loss import CosLinear, CosLoss, CenterLoss
        ...
        if config.use_center_loss:
            ...
            fr_loss = CosLoss(num_cls = args.number_of_class, alpha=0.35)
        ...
        if config.start_from_warmpixel:
            load_model_m(criterion.loss_3d_func,config.checkpoint_warm_pixel+'_c.pkl')
        ...
        # save_model(criterion,'/data/jdq/model_s/train_warm3d_criterion_26511_Clinear_m.pkl')
        from loss_batch import mixed_loss_FR_batch
    ...
    if not args.verify:
        ...
        evalset_micc = MICCDataSet(root=config.micc_image_root, filelist=config.micc_filelist, transform=transform_train)
    ...
    writer = SummaryWriter(config.result_dir+'tmp_log3/train_recog_' + config.prefix + '_'+ str(config.tik_shape_weight) + '_' + str(config.weight_edge_lm)+ '_' + time.strftime('%m-%d:%H-%M-%S',time.localtime(time.time())) + '/')

    eval_tool = EvalTool(transform = transform_eval, criterion=criterion, tb_writer = writer, batch_size=16*num_gpus)
    ...
    if args.verify:
        ...
        for epoch in range(int(args.epoch)):
            ...
            #     save_model(model, config.result_dir + 'model/train_stage3_epoch_%d_' % epoch + config.prefix + '.pkl')
            #     save_model(criterion, config.result_dir + 'model/train_stage3_epoch_criterion_%d_' % epoch + config.prefix + '.pkl')
            #     save_model_m(optimizer4nn, config.result_dir + 'model/train_stage3_epoch_optimizer4nn_%d_' % epoch + config.prefix + '.pkl')
